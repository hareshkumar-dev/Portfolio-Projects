/**
 * Authentication routes (agent console) — OAuth 2.0 Web Server flow with PKCE.
 *
 * Agents authenticate on Salesforce's own login page (credentials never
 * touch this app). Salesforce redirects back with an authorization code,
 * which the server exchanges for an access token using a PKCE code
...
const CALLBACK_URL = process.env.SF_CALLBACK_URL || 'http://localhost:5005/api/auth/callback';
...
router.get('/callback', async (req, res) => {
  ...
  try {
    ...
    console.log(`Agent authenticated via OAuth: ${identity.username}`);
