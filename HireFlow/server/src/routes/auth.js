/**
 * Authentication routes (recruiter console) — OAuth 2.0 Web Server flow
 * with PKCE.
 *
 * Recruiters authenticate on Salesforce's own login page (credentials
...
const CALLBACK_URL = process.env.SF_CALLBACK_URL || 'http://localhost:5004/api/auth/callback';
const CLIENT_URL = process.env.CLIENT_URL || 'http://localhost:3004';
...
router.get('/callback', async (req, res) => {
  ...
  try {
    ...
    console.log(`Recruiter authenticated via OAuth: ${identity.username}`);
